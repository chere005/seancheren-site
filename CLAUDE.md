...
php tools/mailtest.php you@example.com   # send one test message through lib/mail.php
...
**Three live instances share one source tree**, and **two separate scripts deploy them**. `./deploy.sh` owns **test and production** — `test` (default) → `/test/`; `prod` → the site root; `both` → both; `promote` → copy the *live test* tree onto prod server-side. A bare `./deploy.sh` is test-only, so production is never hit by accident. `./deploy-dev.sh` owns **`/dev/` and nothing else**: its destinations are constants with a guard that refuses any non-`/dev` path, so it cannot reach test or production, and `deploy.sh` names no `dev` path at all. That split is deliberate — someone working on `/dev/` and someone working on test/prod must not be able to collide. Both lint first, never send a `config.php`, never touch a data dir and never `--delete`. **`deploy-dev.sh` ships a clean `git` checkout, not the working tree** (`--ref <commit>`, default HEAD; `--dirty` opts back into the working tree): `deploy.sh`'s behaviour of shipping whatever is in `public/` right now is right for one person and wrong when someone else is mid-edit.
...
**The `/dev/` slot, a second fixed sandbox.** `seancheren.com/dev/` is a third live instance, built the same way — `lib-dev/`, `data-dev/`, `base` = `/dev` — added as one more hardcoded case beside the `/test/` check in every preamble (not a generalized pattern-match; each instance is its own literal branch). `./deploy-dev.sh` pushes to it — its own script, which cannot write test or production; `promote` never reads from it. A slug must not collide with any real page directory: the marker is matched against `__DIR__`, so a page whose own path contains the slug would load the sandbox's `lib-*` from production.

**The three instances have separate logins and separate sessions.** *Accounts:* each sandbox config is standalone, so `app_users()` sees that instance's own `users` plus whatever signed up into its own `accounts.json`. *Sessions:* all three live on one domain, and a cookie set at path `/` is sent to every path beneath it — so the cookie **name** is the only thing that actually separates them. `session_cookie_name()` (`lib/auth.php`) derives one per *sandbox* from `base` (`SCSESS_TEST`, `SCSESS_DEV`); **production deliberately keeps PHP's own `PHPSESSID` and its default session store**, because renaming its cookie would sign everyone out for nothing. `session_store_dir()` gives each sandbox its own `sessions/` inside its own data dir. The sandbox cookie paths are narrowed to their base as well.
...
- **Behaviour lives in `lib/`; a page holds plumbing.** If you can describe a rule in a
  sentence, it belongs in a helper with a test — not inlined in one page's `index.php`,
  where the next page reimplements it half a shade differently.
...
- **Two sessions share this repo.** `git pull --autostash` first, stage explicit paths,
  never `git add -A` — another agent's half-finished work must not ride along on your
  commit.
- **Production is never touched unless Sean says so in that message.** A bare
  ...
  together; a feature goes to test and waits for him before `promote`. Approval for one
  change is not approval for the next.
- **Anything that tests a deploy script must neuter `ssh`/`rsync` in its copy first.** In
  a sibling repo, a run that proved the consent gate worked — by removing the consent
  gate — went on to write production. The `deploy` test area here only reads the scripts
  as text and parses them with `bash -n`, and proves an idiom by running a one-line
  snippet rather than the script itself. Keep it that way.
- **Sean's data is his.** Reading his live data to find a bug is fine and has found real
  ones. Writing to it, reordering his sections, or seeding over his account is not.
  `tools/seed-accounts.php` touches the demo accounts and nothing else.

### Traps that have cost real time

- **A check that cannot fail looks exactly like one that passes.** Break the thing a new
  test guards and watch it go red before trusting it. This has already bitten here: the
...
- **The shell's working directory persists between Bash calls**, and a `cd` inside one
  announces nothing. Use absolute paths, or the next command runs somewhere you did not
  mean.
- **Comments state intent; the code may have drifted.** Trust the code and fix the
  comment.
