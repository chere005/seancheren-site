...
There was a third instance, `/dev/`, with its own `deploy-dev.sh` that could reach nothing else. Both were removed on 2026-08-23 (Sean: it "shouldn't even exist anymore"). `data-dev` had never been created, so no data moved. **`dev.seancheren.com` is still an ALIAS on the hosting account** — removing that is a control-panel action nobody here can do, and until it happens the subdomain resolves to this same docroot and serves production rather than a sandbox.
